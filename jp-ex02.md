...
5. **ノードプール**　画面に遷移するので、各項目を以下のように設定します
    ...
    **ノードプール**
    ...
    今回は、学習目的でコストを下げるためノード数をより少なくしたいので、既定で追加されている **userpool** にチェックをつけ \[**削除**\] ボタンをクリックして削除します。
...
6. **ネットワーク** タブ画面に遷移するので、各項目を以下のように設定します
    ...
    |Kubernetes サービスのアドレス範囲 \*|`172.16.0.0/16`(※2)|
    |Kubernetes DNS サービスの IP アドレス|`172.16.0.10`(※3)|
    ...
    (※2) Kubernetes サービスのアドレス範囲は、仮想ネットワークのサブネットのアドレス空間と重複しないように設定する必要があります。既定で設定される `10.10.0.0/16` は仮想ネットワークのサブネットのアドレス空間と重複するため、今回の例では `172.16.0.0/16` を指定しています。
    
    (※3) Kubernetes DNS サービスの IP アドレスは、Kubernetes サービスのアドレス範囲内で指定します。
